...
| ID | Feature | Notes |
|----|---------|-------|
| P | ~~2FA~~ — **BUILT (not tested) 2026-07-22** | Shipped as **email-code** 2FA (not TOTP/QR/backup codes — user chose email codes, everyone opt-in, no backup codes). Backend: `users.is_2fa_enabled` + `two_factor_codes` table (migration `2fa7c1d3e5b9`), 6-digit hashed single-use codes (10-min expiry, newest-only), login returns a pending-token challenge exchanged at `/api/auth/login/verify-2fa`; endpoints for setup/confirm/disable + superadmin `/users/{id}/disable-2fa` and `clear_2fa.py` recovery script for lockouts (no backup codes by design). Frontends: code step on both login pages, enable/disable in both settings pages. Covered by `tests/test_two_factor.py` (11 tests). On branch `feat/2fa`. Design: `plans` 2FA plan. **Needs manual end-to-end test** (real email code) before marking tested. |
...
| Y | Validate URL/image-typed override values | The Page Content editor infers a field's input type (text/image/link) from its name, and the backend's `save_override` only allow-lists field *names*, not value content — no length/content validation on what gets stored. Not exploitable today (only plain-text `title`/`titleHighlight` fields are actually flagged editable anywhere), but `infer_field_type` already classifies `*url`/`*href`-suffixed field names as `"link"`, and override values get spread directly into block props with no escaping (e.g. `<GlowButton href={section.cta_url}>`). The moment a superadmin flags a URL/image-shaped field as shop-admin-editable, an admin could store a `javascript:` URI as a stored-XSS vector with no gate at either layer. Needs either backend scheme validation in `save_override` or storefront-side sanitization at render — flagged during Task 13's code review, deliberately not built as part of that task since it's not exploitable under the current config and touches backend/storefront code beyond that task's scope. |
...
| ~~Variant images~~ | **Built, not tested** — `variant_id` on `ProductImage` (nullable, indexed, **not a real FK** — an app-level tag only, e.g. `delete_variant()` must null it explicitly rather than relying on a DB cascade). Admin: Details tab has a per-image "Link image to a variant" dropdown → `PATCH /api/products/{id}/images/{imageId} {variant_id}`. Storefront (`product-detail-client.tsx`): picking a variant swaps the main photo to the first image tagged to it; if that variant has no tagged image, the display does **not** revert to the primary photo, it just stays as-is. The thumbnail strip always shows the first 4 of the product's *entire* image list regardless of variant tagging. Listing/grid/cart/wishlist cards ignore variant tagging entirely — always show the primary/first image. Migration: `e2f3a4b5c6d7`. |
