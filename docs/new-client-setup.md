...
| Browser shows "Your connection is not private" on `https://www.yourdomain.com` (but the plain domain and `admin.` both work fine) | A `www` DNS record was added — either from the start or later, e.g. by a registrar default — but the certificate was issued only for `yourdomain.com` + `admin.yourdomain.com`. `scripts/setup-https.sh`/`scripts/multiclient-add-client.sh` now fold `www` into the cert automatically **when they run and it already resolves here** — this only bites an *existing* deployment where `www` showed up after the cert was already issued. | Re-issue the certificate to add the `www` SAN: `docker compose run --rm certbot certonly --webroot -w /var/www/certbot -d yourdomain.com -d admin.yourdomain.com -d www.yourdomain.com --cert-name yourdomain.com --email you@youragency.com --agree-tos --no-eff-email --non-interactive --force-renewal`. Requires `nginx/default.conf` to already have the `www.${DOMAIN}` server block (redirects to the apex domain) — pull the latest repo version if it predates this, and copy it onto the VPS if you edited it locally. **Then `docker compose restart nginx`, not just `nginx -s reload`** — `default.conf` is an `envsubst` template (`${DOMAIN}` gets substituted into `/etc/nginx/conf.d/default.conf`), and that substitution only runs once, at container **start** (see `nginx/entrypoint.sh`). `nginx -s reload` re-reads whatever was already rendered there — it picks up a renewed *certificate* fine, but not a *config* change, so the new `www` server block silently never takes effect until the container actually restarts. |
| Login (or any admin/storefront action) just silently fails — no error page, and `docker compose logs backend` shows **no matching request at all** | The frontend was rebuilt/recreated (e.g. after changing ports or re-running `generate-env.sh`) while root `.env`'s `NEXT_PUBLIC_API_URL_STOREFRONT`/`NEXT_PUBLIC_API_URL_ADMIN` still pointed at the raw `http://SERVER_IP:PORT` instead of `https://yourdomain.com`. That URL is baked into the JS bundle at build time. Since the page itself loads over HTTPS, the browser silently blocks the plain-`http://` request as **mixed content** — it never leaves the browser, so it never reaches the backend. Check DevTools → Console for a "Mixed Content" warning to confirm. | Fix `NEXT_PUBLIC_API_URL_STOREFRONT`/`NEXT_PUBLIC_API_URL_ADMIN` in root `.env` to `https://yourdomain.com`, then rebuild **and** recreate both frontends so the new bundle is actually used: `docker compose build frontend-starter frontend-admin && docker compose up -d --force-recreate frontend-starter frontend-admin`. |
| A service starts returning `502` / nginx logs `connect() failed (113: Host is unreachable)` right after you rebuild or `--force-recreate` any container that sits behind the bundled nginx | nginx resolves its upstream's (`backend`/`frontend-starter`/`frontend-admin`) hostname to a container IP once, at its own startup, and caches it. Recreating a container gives it a **new** IP — nginx has no way to know and keeps trying the old, now-dead one. | `docker compose exec nginx nginx -s reload` after recreating *any* container nginx proxies to. (`scripts/setup-https.sh` already does this automatically after its own rebuild step — this only bites you on a manual `docker compose up -d --force-recreate <service>` outside the script.) |
