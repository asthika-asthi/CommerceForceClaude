...
export default function MediaPage() {
  ...
  const deleteMutation = useMutation({
    mutationFn: (filename: string) => api.del(`/api/media/files/${encodeURIComponent(filename)}`),
    onSuccess: () => {
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-4">
        {files.map(f => (
          <div key={f.filename} className="bg-white rounded-xl border border-slate-200 overflow-hidden group">
            ...
            <div className="p-2.5">
              <p className="text-xs text-slate-700 font-medium truncate" title={f.filename}>{f.filename}</p>
